...
void GameEngine::updateMove(int &x1, int &x2, int &y1, int &y2) const
{
    ...
    if (Keyboard::isKeyPressed(Keyboard::A) && x1 != board.size)
    {
        x1 = -board.size, y1 = 0;
    }
    if (Keyboard::isKeyPressed(Keyboard::D) && x1 != -board.size)
    {
        x1 = board.size, y1 = 0;
    }
    if (Keyboard::isKeyPressed(Keyboard::W) && y1 != board.size)
    {
        x1 = 0, y1 = -board.size;
    }
    if (Keyboard::isKeyPressed(Keyboard::S) && y1 != -board.size)
    {
        x1 = 0, y1 = board.size;
    }

    // player2 moves with arrow keys
    if (Keyboard::isKeyPressed(Keyboard::Left) && x2 != board.size)
    {
        x2 = -board.size, y2 = 0;
    }
    if (Keyboard::isKeyPressed(Keyboard::Right) && x2 != -board.size)
    {
        x2 = board.size, y2 = 0;
    }
    if (Keyboard::isKeyPressed(Keyboard::Up) && y2 != board.size)
    {
        x2 = 0, y2 = -board.size;
    }
    if (Keyboard::isKeyPressed(Keyboard::Down) && y2 != -board.size)
    {
        x2 = 0, y2 = board.size;
    }
}
...
void GameEngine::checkWinner()
{
    ...
    if (player1.covered > player2.covered)
    {
        player1.score++;
    }
    else if (player2.covered > player1.covered)
    {
        player2.score++;
    }
}
